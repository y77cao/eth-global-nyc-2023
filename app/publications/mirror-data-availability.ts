...
const createDAMirrorViaDispatcherRequest = async (
...
export const createDAMirrorTypedData = async (
  request: CreateDataAvailabilityMirrorRequest
) => {
  const result = await apolloClient.mutate({
...
const mirrorDA = async (
  ...
  if (profileResult.dispatcher?.canUseRelay && useDispatcherIfEnabled) {
    const dispatcherResult = await createDAMirrorViaDispatcherRequest(
      createMirrorRequest
    );
    console.log(
    ...
    console.log(
      "create DA mirror via broadcast: broadcastResult",
      broadcastResult
    );
...
export const createMirrorDataAvailability = async (
  useDispatcherIfEnabled: boolean
) => {
