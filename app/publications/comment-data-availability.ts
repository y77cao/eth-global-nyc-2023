...
export const createDACommentTypedData = async (
  request: CreateDataAvailabilityCommentRequest
) => {
  const result = await apolloClient.mutate({
...
const commentDA = async (
  ...
  if (profileResult.dispatcher?.canUseRelay && useDispatcherIfEnabled) {
    const dispatcherResult = await createDACommentViaDispatcherRequest(
      createDACommentRequest
    );
    console.log(
    ...
    const signedResult = await signCreateDACommentTypedData(
      createDACommentRequest
    );
    ...
    if (
      broadcastResult.__typename !== "CreateDataAvailabilityPublicationResult"
    ) {
    ...
    console.log(
      "create DA comment via broadcast: broadcastResult",
      broadcastResult
    );
...
export const createCommentDataAvailability = async (
  useDispatcherIfEnabled: boolean
) => {
  ...
  const createDACommentRequest: CreateDataAvailabilityCommentRequest = {
    ...
    contentURI:
      "https://arweave.net/A492V5l-4w43v0QbqQZ2vtivOuRxEHTUHBJvub68CZE",
  };

  const result = await commentDA(
    createDACommentRequest,
    useDispatcherIfEnabled
  );
