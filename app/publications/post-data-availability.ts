...
const createDAPostViaDispatcherRequest = async (
...
export const createDAPostTypedData = async (
  request: CreateDataAvailabilityPostRequest
) => {
  const result = await apolloClient.mutate({
...
export const signCreateDAPostTypedData = async (
  request: CreateDataAvailabilityPostRequest
) => {
...
const postDA = async (
  ...
  if (profileResult.dispatcher?.canUseRelay && useDispatcherIfEnabled) {
    const dispatcherResult = await createDAPostViaDispatcherRequest(
      createDAPostRequest
    );
    console.log(
    ...
    console.log(
      "create DA post via broadcast: broadcastResult",
      broadcastResult
    );
...
export const createPostDataAvailability = async (
  useDispatcherIfEnabled: boolean
) => {
